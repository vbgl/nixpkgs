{ stdenv, fetchurl, gdal, cmake, qt4, flex, bison, proj, geos, xlibsWrapper, sqlite, gsl, grass
, qwt, fcgi, pythonPackages, libspatialindex, libspatialite, qscintilla, postgresql, makeWrapper }:

stdenv.mkDerivation rec {
  name = "qgis-2.10.1";

  buildInputs = [ gdal grass qt4 flex bison proj geos xlibsWrapper sqlite gsl qwt qscintilla
  ...
  cmakeFlags = [ "-DGRASS_PREFIX7=${grass}/${grass.name}" ];
